...
test('catalog keeps seven dense puzzles with 12 globally unique answers each',()=>{
  ...
  for(const puzzle of puzzles){assert.equal(puzzle.cols,7);assert.equal(puzzle.rows,10);assert.equal(puzzle.words.length,12);assert.equal(inspect(puzzle).size,70);}
});
