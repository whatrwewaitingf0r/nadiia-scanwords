...
test('catalog keeps six repaired 10 by 7 boards',()=>{
  ...
  for(const puzzle of puzzles.slice(0,6)){assert.equal(puzzle.cols,10);assert.equal(puzzle.rows,7);assert.equal(puzzle.words.length,8);assert.equal(inspect(puzzle).size,70);assert.ok(puzzle.blocks.length>=1);}
});
