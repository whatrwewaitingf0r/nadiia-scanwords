...
const output = path.join(__dirname, '..', 'output', 'v14');
...
(async () => {
  ...
  for (const scenario of scenarios) {
    ...
    assert.equal(initial.version,'v14'); assert.equal(initial.grid.cols,10); assert.equal(initial.grid.rows,7); assert.equal(initial.grid.clues,12); assert.equal(initial.grid.words,12); assert.ok(initial.grid.blocks>=2); assert.equal(initial.grid.letters+initial.grid.clues+initial.grid.blocks,70);
    ...
    const stored=await page.evaluate(()=>localStorage.getItem('nadiia-scanwords-v14')); assert.ok(stored&&stored.includes('scanword-001'));
  ...
  await browser.close(); console.log('Browser smoke: v14 10×7 grid, zoom, bottom dock, persistence, and completion passed.');
