...
const output = path.join(__dirname, '..', 'output', 'v13');
...
(async () => {
  ...
  for (const scenario of scenarios) {
    ...
    assert.equal(initial.version,'v13'); assert.deepEqual(initial.grid,{cols:10,rows:7,letters:56,clues:10,blocks:4,words:10});
    ...
    for(let puzzleIndex=0;puzzleIndex<6;puzzleIndex++){
      const clipped=await page.evaluate(()=>[...document.querySelectorAll('.clue-part')].map(part=>{const text=part.querySelector('.clue-text'),arrow=part.querySelector('.clue-arrow');return {clue:text.textContent,clipped:text.scrollHeight>text.clientHeight+1||text.scrollWidth>text.clientWidth+1||text.getBoundingClientRect().bottom>arrow.getBoundingClientRect().top-1}}).filter(entry=>entry.clipped));
    ...
    const stored=await page.evaluate(()=>localStorage.getItem('nadiia-scanwords-v13')); assert.ok(stored&&stored.includes('scanword-001'));
  ...
  await browser.close(); console.log('Browser smoke: v13 10×7 grid, zoom, bottom dock, persistence, and completion passed.');
